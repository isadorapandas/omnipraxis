...
- Each scene lives under `src/scenes/<slug>/scene.tsx`, with scene-local supporting modules beside it (for example, `pressure-gauge/pressure.ts`). Keep `sceneManifest.ts`, `sceneRegistry.ts`, and `sceneRoute.ts` at `src/scenes/`. Scene components retain descriptive named exports such as `CircuitBreakerScene`, which owns its background/lights, scene asset URLs, repair state, effect configs, and player spawn call.
