...
// com eslint no convai store e sem eslint aqui
...
export function Model(props: ThreeElements['group']) {
  ...
  useFrame((_, delta) => {
    ...
    const bq = client.blendshapeQueue;
    let frameData = null;
    let targetJawOpen = 0; 

    const rawIsTalking = bq?.botSpeaking || client.state?.isCharacterTalking;
...
// com eslint aqui e sem eslint no convai store
...
import { useConvaiStore } from './src/runtime/convai/useConvaiStore'
...
export function Model(props: ThreeElements['group']) {
  ...
  useFrame((_, delta) => {
    ...
    // @ts-ignore - Accessing private properties, permission for custom lip-syncing
    const rawIsTalking = client.blendshapeQueue?.botSpeaking || client.state?.isCharacterTalking;
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore - Accessing the Convai SDK private queue
    const bqQueue = client.blendshapeQueue?.queue;

    if (bqQueue) {
      if (!isCharacterTalking) {
        bqQueue.length = 0;
      } else if (bqQueue.length > 0) {
        frameData = bqQueue.shift(); 
        if (bqQueue.length > 12) bqQueue.shift();
      }
    }
